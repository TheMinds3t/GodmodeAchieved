...
monster.npc_update = function(self, ent, data, sprite)
    ...
    if ent.SubType == 1 then --apple!
        if sprite:IsFinished("Appear") then 
            ...
            sprite:Play("GlowAppear",true)
        end

        if sprite:IsFinished("GlowAppear") then 
        ...
        data.anchor_pos = data.anchor_pos or GODMODE.room:GetGridPosition(GODMODE.room:GetGridIndex(ent.Position))
        ...
        elseif ent.I1 >= monster.states.mad then 
            if sprite:IsOverlayFinished("HeadConvert") then 
                sprite:PlayOverlay("Head2Idle",false)
            end

            if sprite:IsOverlayFinished("Head2Idle") then 
                if ent:GetDropRNG():RandomFloat() < 0.6+math.min(0.4,0.4-ent.I2) or player == nil or not monster:should_attack(ent,player) then 
                    sprite:PlayOverlay("Head2Idle",true)
                    ent.I2 = ent.I2 + 1
                else
                    ...
                    if ent.Position.Y > player.Position.Y then 
                        sprite:PlayOverlay("AttackUp",true)
                    else
                        sprite:PlayOverlay("AttackDown",true)
                    end
                end
            end

            if sprite:IsOverlayFinished("AttackUp") or sprite:IsOverlayFinished("AttackDown") then 
                sprite:PlayOverlay("Head2Idle",true)
        ...
        if data.apple_obj ~= nil and ent.I1 ~= monster.states.mad then 
            if data.apple_obj:IsDead() then data.apple_obj = nil data.apple_target = nil else 
                ...
                if dist<(data.apple_obj.Size+ent.Size)*3 and GetPtrHash(GODMODE.get_ent_data(data.apple_obj).veng_obj or player) == GetPtrHash(ent) then 
                    if sprite:GetOverlayAnimation() ~= "HeadConvert" then 
                        sprite:PlayOverlay("HeadConvert",true)
                        sprite:Play("BodyConvert",true)
                    end
    
                    ent.I1 = monster.states.transition
                elseif not sprite:GetOverlayAnimation() == "Head" then  
        ...
        if sprite:IsOverlayPlaying("AttackUp") or sprite:IsOverlayPlaying("AttackDown") then 
            ent.FlipX = false
            local up = sprite:IsOverlayPlaying("AttackUp")
