    ...
    ["Wired"]={
        ...
        touch_effect = function(ent,player) 
            ...
            if slot > -1 and player:GetActiveItem(slot) > 0 and player:GetActiveCharge(slot) > 0 and ent.I2 == 0 then 
                ...
                GODMODE.sfx:Play(SoundEffect.SOUND_BATTERYDISCHARGE)
                ent:Kill()
                monster.npc_kill(nil,ent)
            end
            return true
...
monster.npc_update = function(self, ent, data, sprite)
    ...
    if ent.SubType == 0 then 
        if not sprite:IsPlaying(profile.alive_anim) then 
            sprite:Play(profile.alive_anim,false)
        end
        ...
        sprite:Play(profile.dead_anim,false)
...
monster.npc_kill = function(self, ent)
    local door = Isaac.Spawn(monster.type,monster.variant,1,ent.Position,Vector.Zero,nil):ToNPC()
    door:ClearEntityFlags(EntityFlag.FLAG_APPEAR)
    local data = GODMODE.get_ent_data(door)
    door.I1 = ent:ToNPC().I1
