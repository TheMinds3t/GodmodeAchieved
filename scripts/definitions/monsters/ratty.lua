...
local pickup_predicate = function(ent) 
    ...
    ent.Variant == PickupVariant.PICKUP_KEY or ent.Variant == PickupVariant.PICKUP_BOMB or ent.Variant == PickupVariant.PICKUP_PILL or ent.Variant == Isaac.GetEntityVariantByName("Fruit (Pickup)"))
...
monster.npc_update = function(self, ent)
    ...
    if ent:GetSprite():IsPlaying("Appear") or ent.FrameCount < 2 then 
        ent:GetSprite().PlaybackSpeed = 1.0
        return
    elseif not ent:GetSprite():IsPlaying("DigIn") or ent.I1 == 0 then 
        if ent.Velocity:Length() > 0.25 then 
            if math.abs(ent.Velocity.X) > math.abs(ent.Velocity.Y) then 
                if ent.Velocity.X > 0 then 
                    ent:GetSprite():Play("WalkLeft",false)
                else
                    ent:GetSprite():Play("WalkRight",false)
                end
            else
                if ent.Velocity.Y > 0 then 
                    ent:GetSprite():Play("WalkDown",false)
                else
                    ent:GetSprite():Play("WalkUp",false)
                end
            end    
        end

        ent:GetSprite().PlaybackSpeed = math.min(1.2,ent.Velocity:Length()/4/ent.Scale)
    ...
    if data.cur_pickup ~= nil then 
        if data.cur_pickup:IsDead() then 
            ...
            if (player.Position - ent.Position):Length() > rat_tunnel_dist then 
                if ent.I1 > 100 and not ent:GetSprite():IsPlaying("DigIn") then 
                    ent:GetSprite():Play("DigIn",true)
                end
    ...
    -- if ent:GetSprite():IsEventTriggered("Fall") and data.cur_pickup ~= nil then 
...
monster.npc_kill = function(self,ent)
    ...
    if pickup ~= nil then 
        ...
        -- pickup.Position = Game():GetRoom():FindFreePickupSpawnPosition(pickup.Position)
