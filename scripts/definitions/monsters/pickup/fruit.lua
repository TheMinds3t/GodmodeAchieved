...
local num_subs = 5
local stat_ups = {
    [subtypes.grapes] = {flag = CacheFlag.CACHE_SPEED, amt = 0.05},
    [subtypes.orange] = {flag = CacheFlag.CACHE_FIREDELAY, amt = 0.1},
    [subtypes.apple] = {flag = CacheFlag.CACHE_DAMAGE, amt = 0.25},
    [subtypes.banana] = {flag = CacheFlag.CACHE_SHOTSPEED, amt = 0.05},
    [subtypes.kiwi] = {flag = CacheFlag.CACHE_RANGE, amt = 10},
}
...
monster.pickup_init = function(self,ent)
    ...
    if ent.State == 0 then--Game():GetRoom():IsFirstVisit() or ent.State == 0 then 
    ...
    for _,sub in pairs(subtypes) do 
        if sub > 0 then
            ...
            if cache == f_cache then 
                ...
                -- end
                GODMODE.util.modify_stat(player, cache, amt)
            end
        end
    end
end
...
monster.player_update = function(self, player)
    ...
    for _,sub in pairs(subtypes) do 
        if sub > 0 then 
            ...
            local perc = (Game():GetFrameCount() - tonumber(GODMODE.save_manager.get_player_data(player,"TimeStamp"..sub,"0"))) / duration
            ...
            if perc < 1.0 and max_amt > 0 then 
                ...
                GODMODE.save_manager.set_player_data(player,"MaxFruit"..sub,0,true)
...
monster.player_collide = function(self, player,ent,entfirst)
    if (ent.Type == monster.type and ent.Variant == monster.variant) then 
        ...
        if not ent:GetSprite():IsPlaying("Collect") then
            ...
            GODMODE.save_manager.set_player_data(player,"TimeStamp"..ent.SubType,Game():GetFrameCount(),true)
