...
if not (ent.Type == monster.type and ent.Variant == monster.variant) then return end	local data = GODMODE.get_ent_data(ent)
    ...
    if data then
        if not data.init then
            ...
            data.tears = {}
            data.tear_time = {}
            data.slowed = GODMODE.util.count_enemies(nil, Isaac.GetEntityTypeByName("The Grand Marshall"), Isaac.GetEntityVariantByName("The Grand Marshall")) ~= 0
            ...
            if ent:GetSprite():IsEventTriggered("Ring") then
                SFXManager():Play(SoundEffect.SOUND_CHILD_ANGRY_ROAR)
                SFXManager():AdjustPitch(SoundEffect.SOUND_CHILD_ANGRY_ROAR,0.7)
                SFXManager():Play(SoundEffect.SOUND_HEARTOUT,0.5)
                SFXManager():AdjustPitch(SoundEffect.SOUND_HEARTOUT,0.7)
            ...
            for i,tear in pairs(data.tears) do
                ...
                if tear_time == 15 then
                    ...
                    SFXManager():Play(SoundEffect.SOUND_HEARTIN,0.15)
                    SFXManager():AdjustPitch(SoundEffect.SOUND_HEARTIN,0.7)
