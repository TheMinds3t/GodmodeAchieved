...
monster.npcUpdate = function(self, ent)
    ...
    if data.minions == nil then
        ...
        data.spawn_tear = function(self, ang, speed, curve)
            ...
            local tear = Game():Spawn(EntityType.ENTITY_PROJECTILE,0,ent.Position + vel,vel,ent,0,ent.InitSeed)
    ...
    if data.minions / data.max_minions > f then
        ...
        Game():Spawn(38,0,ent.Position,Vector(ent:GetDropRNG():RandomFloat()*4-2,ent:GetDropRNG():RandomFloat()*4-2),ent,800,ent.InitSeed)
    end
    ...
    if ent:GetSprite():IsEventTriggered("Finale") then
        for i=0,11 do
            Game():Spawn(38,0,ent.Position,Vector(ent:GetDropRNG():RandomFloat()*50-25,ent:GetDropRNG():RandomFloat()*50-252),ent,800,ent.InitSeed)
        end
        ent:Kill()
        Game():ShakeScreen(20)
        Game():GetRoom():EmitBloodFromWalls(5,60)
    end
    ...
    if ent:GetSprite():IsPlaying("Attack2") then
        Game():GetRoom():EmitBloodFromWalls(1,1)
        Game():ShakeScreen(1)
