...
monster.npc_update = function(self, ent)
    ...
    if ent:GetSprite():IsEventTriggered("OpenSound") then
    ...
    if GODMODE.util.is_delirium() and data.opened ~= true then 
        data.keys_left = 0
        ent:GetSprite():Play("Open",false)
        ent:GetSprite().PlaybackSpeed = 2.5
    end

    if data.keys_left == 0 then
        if ent:GetSprite():IsFinished("UseKey") or ent:GetSprite():IsFinished("UseGoldenKey") then
            ent:GetSprite():Play("Open",true)
        end    

        if ent:GetSprite():IsEventTriggered("Open") then
            data.opened = true
            Game():ButterBeanFart(ent.Position,64,ent,true,true)
        ...
        if ent:GetSprite():IsFinished("Open") or ent:GetSprite():IsFinished("Attack") then
            ent:GetSprite():Play("IdleOpen",false)
            ent.CollisionDamage = 1
            ent:GetSprite().PlaybackSpeed = 1
        end

        if ent:GetSprite():IsPlaying("IdleOpen") and data.time % 30 == 0 then
        ...
        if ent:GetSprite():IsEventTriggered("Grunt") then
            data.grunt_tell = data.grunt_tell + 1
            if data.grunt_tell == 1 then
                SFXManager():Play(SoundEffect.SOUND_MONSTER_ROAR_2)
            else
                SFXManager():Play(SoundEffect.SOUND_MONSTER_YELL_B)
            end
        end
        ...
        if Isaac.GetPlayer():GetNumKeys() < data.keys_left and not Game():GetRoom():IsClear() then
...
monster.npc_kill = function(self, ent)
    Game():BombExplosionEffects(ent.Position,30)
    local pickups = {
    ...
    for i=1,12+ent:GetDropRNG():RandomInt(9) do
    ...
    Isaac.Spawn(EntityType.ENTITY_PICKUP,PickupVariant.PICKUP_COLLECTIBLE,0,Game():GetRoom():FindFreePickupSpawnPosition(ent.Position),Vector.Zero,ent)
    Isaac.Spawn(EntityType.ENTITY_PICKUP,PickupVariant.PICKUP_COLLECTIBLE,0,Game():GetRoom():FindFreePickupSpawnPosition(ent.Position),Vector.Zero,ent)
