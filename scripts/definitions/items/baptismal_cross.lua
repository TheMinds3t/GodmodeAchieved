local item = {}
item.instance = GODMODE.registry.items.baptismal_cross
			...
			if time then
				if time.time > 0 then
					...
					for i=1,2 do
						...
						local t = Isaac.Spawn(EntityType.ENTITY_TEAR,0,0,player.Position,Vector(math.cos(ang)*(player:GetCollectibleRNG(item.instance):RandomFloat() * 10),math.sin(ang)*(player:GetCollectibleRNG(item.instance):RandomFloat() * 10)),player)
						t = t:ToTear()
