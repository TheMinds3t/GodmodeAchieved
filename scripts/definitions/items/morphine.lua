...
item.eid_description = "↑ +1.0 Damage#↓ -20% Speed#↑ 30% chance to negate damage taken#↓ +2 Broken hearts"
item.eid_transforms = GODMODE.util.eid_transforms.SPUN
item.encyc_entry = {
	{ -- Effects
		{str = "Effects", fsize = 2, clr = 3, halign = 0},
		{str = "Grants +1.0 damage, -20% speed and 2 broken hearts on picking it up."},
...
item.eval_cache = function(self, player,cache)
    ...
    data.num_morphine_brokens = (tonumber(GODMODE.save_manager.get_player_data(player, "MorphineBrokens", "0")))

    if data.num_morphine_brokens == nil or data.num_morphine_brokens < player:GetCollectibleNum(item.instance) then
        player:AddBrokenHearts(2)
        data.num_morphine_brokens = data.num_morphine_brokens + 1
        GODMODE.save_manager.set_player_data(player, "MorphineBrokens", data.num_morphine_brokens,true)
    end

    if cache == CacheFlag.CACHE_DAMAGE then
        player.Damage = player.Damage + 1*(player:GetCollectibleNum(item.instance))
    end
    ...
    if enthit:ToPlayer() and enthit:ToPlayer():HasCollectible(item.instance) and amount > 0 then
        ...
        if player:HasCollectible(item.instance) and (player:GetCollectibleRNG(item.instance):RandomFloat() < 0.33 or (data.morphine_cooldown or 0) > 0) then
            if data.morphine_cooldown == nil or data.morphine_cooldown <= 0 then 
                ...
                SFXManager():Play(SoundEffect.SOUND_ANIMAL_SQUISH, Options.SFXVolume+0.2, 20)
                SFXManager():Play(SoundEffect.SOUND_BLOBBY_WIGGLE, Options.SFXVolume+0.2)
