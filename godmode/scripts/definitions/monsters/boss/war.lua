...
monster.npc_update = function(self, ent)
    ...
    if ent:GetSprite():IsEventTriggered("Fire") then 
        ...
        if ent:GetSprite():IsPlaying("Dash") and room:IsPositionInRoom(ent.Position,16) then
    ...
    if ent.HitPoints / ent.MaxHitPoints < 0.5 then 
        ...
        war:GetSprite():ReplaceSpritesheet(0, "godmode/gfx/bosses/horsemen/boss_000_bodies02.png")
        war:GetSprite():ReplaceSpritesheet(1, "godmode/gfx/bosses/horsemen/boss_052_war_nest.png")
...
monster.bomb_init = function(self,bomb)
    if bomb.SpawnerEntity ~= nil and bomb.SpawnerEntity.Type == monster.type and bomb.SpawnerEntity.Variant == monster.variant then 
        ...
        local num_burrow = GODMODE.util.count_enemies(nil,Isaac.GetEntityTypeByName("Outbreak"),Isaac.GetEntityVariantByName("Outbreak"),2) 
        ...
        if (num < 3 or num_spider < 3) and num_burrow < 3 then 
            ...
            local burrow = Isaac.Spawn(Isaac.GetEntityTypeByName("Outbreak"),Isaac.GetEntityVariantByName("Outbreak"),2,pos,Vector.Zero,bomb.SpawnerEntity)
...
monster.npc_init = function(self,ent)
    if ent.Type == monster.type and ent.Variant == 0 then 
        if StageAPI ~= nil and StageAPI.GetCurrentStage() ~= nil and StageAPI.GetCurrentStage().Name == "TheNest" or ent:GetDropRNG():RandomFloat() < tonumber(GODMODE.save_manager.get_config("AltHorsemanChance","0.2")) then 
