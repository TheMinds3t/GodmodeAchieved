...
local max_charge = 200
local max_submerge = 300
...
local splash = function(ent,vel,pos_off)
    ...
    SFXManager():Play(SoundEffect.SOUND_BOSS2INTRO_WATER_EXPLOSION,Options.SFXVolume * 2.5,1,false,1.2+ent:GetDropRNG():RandomFloat()*0.5)
    Game():ShakeScreen(5)
end
...
local fire_tear = function(ent,ang,speed,scale)
    ...
    if ent.SubType == 1 and ent:HasEntityFlags(EntityFlag.FLAG_FRIENDLY) then 
        tear = Game():Spawn(EntityType.ENTITY_TEAR,0,ent.Position + vel,vel,ent,0,ent.InitSeed)
        tear = tear:ToTear()
        ...
        tear = Game():Spawn(EntityType.ENTITY_PROJECTILE,ProjectileVariant.PROJECTILE_TEAR,ent.Position + vel,vel,ent,0,ent.InitSeed)
...
monster.npc_update = function(self, ent)
    ...
    if ent.SubType == 0 then 
        ...
        if ent:GetSprite():IsFinished("Appear") or ent:GetSprite():IsFinished("Attack4") or ent:GetSprite():IsFinished("Attack1") or ent:GetSprite():IsFinished("Attack2Leave") or ent:GetSprite():IsFinished("Attack3End") then
        ...
        if (data.submerge_time or -1) == 0 then 
            ent:GetSprite():Play("Attack2Leave",true) 
        end
        ...
        if (data.charge_time or -1) == 0 then 
            ent:GetSprite():Play("Attack3End",true) 
        end

        if ent:GetSprite():IsPlaying("Attack3End") then 
            ent.Velocity = ent.Velocity * 0.85
        end

        if ent:GetSprite():IsFinished("Attack2Enter") and (data.submerge_time or 0) < 0 then 
            data.submerge_time = max_submerge
        end

        if ent:GetSprite():IsFinished("Attack3") then 
        ...
        if (data.squish_time or 0) > 0 then
            ...
            -- ent:GetSprite().PlaybackSpeed = 1.0 + data.squish_time / 40.0
        elseif ent:GetSprite().PlaybackSpeed > 1 then  
        ...
        if (data.charge_time or 0) > 0 then 
            ...
            if ent.Velocity.Y < 0 then 
                ent:GetSprite():Play("Attack3Loop",false)
                cur_dir.Y = -1
            else
                ent:GetSprite():Play("Attack3BackLoop",false)
                cur_dir.Y = 1
            end
            ...
            if data.charge_time % 7 == 0 or data.charge_time % 6 == 0 and data.charge_time % 42 ~= 0 then 
                ...
                SFXManager():Play(SoundEffect.SOUND_BOSS2_BUBBLES,Options.SFXVolume * 3,1,false,2.2+ent:GetDropRNG():RandomFloat()*0.5)
            end
        ...
        elseif not ent:GetSprite():IsPlaying("Attack3") then
            ent.FlipX = false
        end
        ...
        if ent:GetSprite():IsEventTriggered("Splash") then 
            splash(ent)
            ...
            if ent:GetSprite():IsPlaying("Attack2Leave") then 
                ent.EntityCollisionClass = EntityCollisionClass.ENTCOLL_ALL
            end
            
            if ent:GetSprite():GetFrame() <= 10 and ent:GetSprite():IsPlaying("Attack2Enter") or ent:GetSprite():IsPlaying("Attack2Leave") then 
                SFXManager():Play(SoundEffect.SOUND_BABY_HURT,Options.SFXVolume * 2.5,1,false,1.0+ent:GetDropRNG():RandomFloat()*0.35)
            end
        end

        if ent:GetSprite():IsPlaying("Attack4") then 
            ent.Velocity = ent.Velocity * 0.5-- + (Game():GetRoom():GetCenterPos() - ent.Position) * (1/20.0)
        end

        if ent:GetSprite():IsPlaying("Attack1") and ent:GetSprite():GetFrame() == 20 then 
            if ent.HitPoints / ent.MaxHitPoints < 0.2 then 
                if (data.fire_loop or 0) < 2 then 
                    ent:GetSprite():SetFrame(4)
                end
        ...
        if ent:GetSprite():IsEventTriggered("Shoot") then 
            if ent:GetSprite():IsPlaying("Attack4") then 
                ...
                SFXManager():Play(SoundEffect.SOUND_BOSS2_BUBBLES,Options.SFXVolume * 2,1,false,2.2+ent:GetDropRNG():RandomFloat()*0.5)

            elseif ent:GetSprite():IsPlaying("Attack1") then 
                ent.Velocity = ent.Velocity * 0.8 - Vector(0,3)
                fire_bubble_tear(ent,90,1.25,1)
                SFXManager():Play(SoundEffect.SOUND_BABY_HURT,Options.SFXVolume * 2.5,1,false,1.0+ent:GetDropRNG():RandomFloat()*0.35)
            elseif ent:GetSprite():IsPlaying("Attack2Enter") or ent:GetSprite():IsPlaying("Attack2Leave") then 
    ...
    if ent.Type == monster.type and ent.Variant == monster.variant then 
        if ent.SubType > 0 then 
            if ent.SubType < 3 then 
                ...
                if ent.I2 ~= 0 then 
                    ...
                    if ent.SubType == 1 then                 
                        -- Game():BombExplosionEffects(ent.Position,0.0,0,ent.SplatColor,ent,0.666,true,false,DamageFlag.DAMAGE_FAKE)
                    end
                end
                ...
                if ent.SubType == 1 then 
                    for i=1,8 do 
                        fire_tear(ent,360/8*i,8.0+Game().Difficulty % 2 * 2,1.25)
                    end
                end
            else
                if ent.SubType == 3 then 
            ...
            if ent:ToNPC().Scale < 1.0 then 
                Isaac.Spawn(EntityType.ENTITY_PICKUP,PickupVariant.PICKUP_COLLECTIBLE,Isaac.GetItemIdByName("Bubble Wand"),Game():GetRoom():FindFreePickupSpawnPosition(ent.Position),Vector.Zero,ent)
            end
            ...
            splash(ent)
            SFXManager():Play(SoundEffect.SOUND_BOSS1_EXPLOSIONS,Options.SFXVolume * 2.5,1,false,1.2+ent:GetDropRNG():RandomFloat()*0.5)
        end

        SFXManager():Play(SoundEffect.SOUND_PESTILENCE_MAGGOT_POPOUT,Options.SFXVolume * 2.5,1,false,1.2+ent:GetDropRNG():RandomFloat()*0.5)
        SFXManager():Play(SoundEffect.SOUND_PESTILENCE_HEAD_EXPLODE,Options.SFXVolume * 2.5,1,false,1.2+ent:GetDropRNG():RandomFloat()*0.5)
    end
end
