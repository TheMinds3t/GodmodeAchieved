...
monster.burrow_health_pool = 200
monster.burrow_health_pool_scaled = 50
monster.max_enemies = 7
...
monster.spawnable_spiders = {
    ...
    -- {type=EntityType.ENTITY_CRAZY_LONG_LEGS,var=1},
    -- {type=EntityType.ENTITY_HOPPER,var=1}, --trite
    -- {type=Isaac.GetEntityTypeByName("Planter"),var=Isaac.GetEntityVariantByName("Planter")},
    -- {type=Isaac.GetEntityTypeByName("Godleg"),var=Isaac.GetEntityVariantByName("Godleg")},
}        
...
monster.spider_logic = function(self,data,ent)
    ...
    if ent:GetSprite():IsPlaying("Idle") or ent:GetSprite():IsPlaying("Walk") then 
...
monster.npc_update = function(self, ent)
    ...
    if ent.SubType == monster.subtypes.boss then --outbreak -------------------------------------------------------------------------
        ...
        ent.HitPoints = math.max(ent.HitPoints,(15 - (data.burrow_count or 0)*3-3))

        monster:spider_logic(data,ent)
        ...
        elseif ent:GetSprite():GetAnimation() == "DigIn" then
        ...
        elseif ent:GetSprite():IsFinished("DigOut") then 
            ent:GetSprite():Play("Idle",true)
        end

        if ent:GetSprite():IsEventTriggered("Emerge") then 
            spawn_rock_fx(ent)
            if ent:GetSprite():IsPlaying("DigIn") then 
        ...
        if ent:GetSprite():IsEventTriggered("Dig") then 
            if ent:GetSprite():IsPlaying("DigIn") then 
                ent.Visible = false
    ...
    elseif ent.SubType ~= monster.subtypes.minion then --burrows -------------------------------------------------------------------------
        if ent:HasEntityFlags(EntityFlag.FLAG_APPEAR) then 
            ...
            if ent.SubType == monster.subtypes.burrow then 
                ent:GetSprite():Play("Burrow",true) 
            else
                ent:GetSprite():Play("BurrowSpawn",true) 
            end

            ent:GetSprite().PlaybackSpeed = 0.6+ent:GetDropRNG():RandomFloat()*0.6
        end 
        ...
        if ent:GetSprite():IsFinished("Burrow") or ent:GetSprite():IsFinished("BurrowSpawn") then 
            ent:Remove()
        end
