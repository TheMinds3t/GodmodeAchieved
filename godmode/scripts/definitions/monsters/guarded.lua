...
if not (ent.Type == monster.type and ent.Variant == monster.variant) then return end	local data = GODMODE.get_ent_data(ent)
    ...
    if data.angels and data.time % 5 == 0 and data.idling then
        if #data.angels > 0 then
            ent:GetSprite():Play("IdleInvul",false)
        else
    ...
    if data.angels then
        if data.angel_time > 100 and data.idling and #data.angels < 2 and ent.FrameCount > 10 then
            if #data.angels > 0 then
                ent:GetSprite():Play("SummonInvul",true)
            else
                ent:GetSprite():Play("Summon",true)
            end
    ...
    local spd = 0.35
    if ent:GetSprite():IsPlaying("Summon") or ent:GetSprite():IsFinished("SummonInvul") then spd = 0.05 end

    local vel = (player.Position - ent.Position):Resized(spd)
    ent.Velocity = ent.Velocity * 0.91 + vel * 0.6
    
    if ent:GetSprite():IsFinished("Summon") or ent:GetSprite():IsFinished("SummonInvul") then
        if #data.angels > 0 then
            ent:GetSprite():Play("IdleInvul",true)
        else
            ent:GetSprite():Play("Idle",true)
        end
...
monster.npc_hit = function(self,enthit,amount,flags,entsrc,countdown)
    if enthit.Type == monster.type and enthit.Variant == monster.variant and GODMODE.util.count_enemies(nil,EntityType.ENTITY_BABY,1,-1,false) > 0 then 
