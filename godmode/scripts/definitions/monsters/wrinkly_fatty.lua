...
monster.npc_update = function(self, ent)
    ...
    if ent:GetSprite():IsEventTriggered("Move") then
        ent.I1 = 10
    end
    ...
    if dir % 2 == 0 then 
        ent:GetSprite():Play("WalkHori",false)
    elseif dir == 1 then
    ...
    if ent:IsFrame(60-ent.InitSeed%16,ent.InitSeed) then 
        ...
        if ent:GetDropRNG():RandomFloat() < 0.5 then 
            SFXManager():Play(SoundEffect.SOUND_MONSTER_ROAR_0,vol,2,false,pitch)
        else
            SFXManager():Play(SoundEffect.SOUND_MONSTER_ROAR_1,vol,2,false,pitch)
        end
    end
end
