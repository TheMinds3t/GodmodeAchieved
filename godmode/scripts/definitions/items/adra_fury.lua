local item = {}
item.instance = GODMODE.registry.items.adramolechs_fury
item.eid_description = "↑ -20 charges to negate damage#↑ Small all stats up for each charge#↓ 50% chance for enemies to become champions#Charges when champions or bosses are killed, or when items are picked up outside angel rooms#↑ On use, teleports to special devil room"
...
item.encyc_entry = {
	{ -- Effects
      ...
      {str = " - You will gain 1 charge for this item for every 30 health on a marked champion enemy killed by any player, or 1 charge for every 50 health on any boss enemy."},
    ...
    { -- Notes
	  ...
	  {str = "Items converted to charge give more charge based on the quality of the item."},
	  {str = "Taking items from devil rooms will add 10 charges on top of the charge you would've gotten already."},
	  {str = "Angel room items do not get converted into charges."},
    },
}
...
item.eval_cache = function(self, player,cache,data)
    ...
    if cache == CacheFlag.CACHE_DAMAGE then
        ...
        local eye_lvl = math.max(0,math.floor((charges-1)/25))

        for i=0,3 do 
            player:TryRemoveNullCostume(GODMODE.registry.costumes.t_xaphan_eyes[i+1])
        end

        player:AddNullCostume(GODMODE.registry.costumes.t_xaphan_eyes[eye_lvl+1])
    end
...
item.indicator:Load("godmode/gfx/effect_adra_fury_champ.anm2", true)
        ...
        if slot > -1 then
            if player:GetActiveItem(slot) == item.instance and player:GetActiveCharge(slot) > 0 or (data.adra_cooldown or 0) > 0 then
                ...
                if player:GetActiveCharge(slot) > 9 and player:GetActiveCharge(slot) < 21 then
                    GODMODE.game:BombExplosionEffects(player.Position,player.Damage * 5 + 40,TearFlags.TEAR_NORMAL,Color.Default,player,1,false)
	...
	if ent:ToPlayer() and (pickup.Variant == PickupVariant.PICKUP_COLLECTIBLE) and pickup.Price < ent:ToPlayer():GetNumCoins() and entfirst == false then
        ...
        if room:GetType() == RoomType.ROOM_ANGEL or room_data.Name == "[GODMODE] Adramolech's Fury" or room_data.Name == "Death Certificate" then
            ...
            if player:HasCollectible(item.instance) then 
                ...
                if player:GetActiveCharge(slot) < 100 and pickup.SubType > 0 and pickup.SubType ~= CollectibleType.COLLECTIBLE_BIRTHRIGHT and pickup.SubType ~= GODMODE.registry.items.adramolechs_blessing then 
                    ...
                    if config.Tags & ItemConfig.TAG_QUEST ~= ItemConfig.TAG_QUEST then 
                        ...
                        if room:GetType() == RoomType.ROOM_DEVIL and pickup.Price ~= 0 then 
                            ...
                            count = count + 10
                            GODMODE.sfx:Play(SoundEffect.SOUND_DEVILROOM_DEAL,Options.SFXVolume*10.0)
                        ...
                        for i=1,count do 
                            local charge = Isaac.Spawn(GODMODE.registry.entities.adramolechs_fuel.type, GODMODE.registry.entities.adramolechs_fuel.variant,1,ent.Position,RandomVector()*(player:GetCollectibleRNG(item.instance):RandomFloat()*2+4.0),pickup)
...
item.npc_remove = function(self, ent)
    ...
    if ent:ToNPC() and (ent:IsBoss() and ent:ToNPC().ParentNPC == nil or data and data.adra_fury == true) then
        local count = math.max(0,math.floor((ent.MaxHitPoints-25)/25))+1
        if ent:IsBoss() then count = math.floor(ent.MaxHitPoints/40) end

        GODMODE.util.macro_on_players_that_have(item.instance, function(player) 
            if not player:IsDead() then 
                for i=1,count do 
                    local charge = Isaac.Spawn(GODMODE.registry.entities.adramolechs_fuel_charged.type, GODMODE.registry.entities.adramolechs_fuel_charged.variant,GODMODE.registry.entities.adramolechs_fuel_charged.subtype,ent.Position,RandomVector()*(player:GetCollectibleRNG(item.instance):RandomFloat()*2+4.0),ent)
                    GODMODE.get_ent_data(charge).player_target = player
