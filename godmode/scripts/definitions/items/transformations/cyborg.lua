local transform = include("godmode.scripts.definitions.items.transformations.transform")
...
transform.use_item = function(self, coll,rng,player,flags,slot,var_data)
	if has_cyborg(player) and slot ~= -1 and coll == player:GetActiveItem(slot) then 
