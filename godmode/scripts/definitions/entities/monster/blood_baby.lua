local monster = {}
monster.name = "[GODMODE] Blood Baby"
monster.type = GODMODE.registry.entities.blood_baby.type
monster.variant = GODMODE.registry.entities.blood_baby.variant
...
monster.npc_update = function(self, ent, data, sprite)
	...
	if sprite:IsEventTriggered("Bleed") then
        ...
        if ent:GetDropRNG():RandomInt(2) == 0 then 
            GODMODE.sfx:Play(SoundEffect.SOUND_BLOODSHOOT)
        else
            GODMODE.sfx:Play(SoundEffect.SOUND_BLOBBY_WIGGLE)
        end
	end
end
