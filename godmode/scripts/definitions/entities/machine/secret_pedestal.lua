...
monster.type = GODMODE.registry.entities.unlock_pedestal.type
monster.variant = GODMODE.registry.entities.unlock_pedestal.variant
monster.secrets_desc_anim = Sprite()
monster.secrets_desc_anim:Load("godmode/gfx/achievements/unlocks.anm2",true)
...
monster.pickup_post_render = function(self, pickup, offset)
	...
	monster.secrets_desc_anim.Scale = Vector(1.0+math.cos(anim_item_time+pickup.Position.X)*0.005,1.0+math.sin(anim_item_time+pickup.Position.Y)*0.005)
...
monster.pickup_update = function(self, pickup, data, sprite)
	...
	local data = GODMODE.get_ent_data(pickup) 
	data.anchor_pos = data.anchor_pos or GODMODE.room:GetGridPosition(GODMODE.room:GetGridIndex(pickup.Position))
