local monster = {}
monster.name = "[GODMODE] Pair of Cans"
monster.type = GODMODE.registry.entities.pair_of_cans.type
monster.variant = GODMODE.registry.entities.pair_of_cans.variant
...
monster.familiar_collide = function(self, fam, enthit, entfirst)
    ...
    if enthit.Type == EntityType.ENTITY_PROJECTILE and not enthit:ToProjectile():HasProjectileFlags(ProjectileFlags.CANT_HIT_PLAYER) and fam.Type == monster.type and fam.Variant == monster.variant then
        enthit:Kill()
        fam.HitPoints = fam.HitPoints - 10 / (fam.Player:GetCollectibleNum(GODMODE.registry.items.pair_of_cans) + fam.Player:GetCollectibleNum(CollectibleType.COLLECTIBLE_BFFS))
