local monster = {}
monster.name = "[GODMODE] Ritual Candle (Familiar)"
monster.type = GODMODE.registry.entities.ritual_familiar.type
monster.variant = GODMODE.registry.entities.ritual_familiar.variant
...
monster.familiar_update = function(self, fam, data)
    if fam.Type == monster.type and fam.Variant == monster.variant then
        ...
        if fam:GetSprite():IsEventTriggered("Attack") and fam.Target ~= nil then 
            ...
            effect.CollisionDamage = 5.0 + fam.Player.Damage * GODMODE.level:GetAbsoluteStage() / 13.0
        end
