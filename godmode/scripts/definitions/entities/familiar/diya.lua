local monster = {}
monster.name = "[GODMODE] Diya Candle"
monster.type = GODMODE.registry.entities.diya.type
monster.variant = GODMODE.registry.entities.diya.variant
...
monster.familiar_update = function(self, fam, data)
    ...
    if player:HasCollectible(GODMODE.registry.items.diya) == false then fam:Remove() end
    ...
    if GODMODE.save_manager.get_player_data(player,"DiyaLit","false") == "true" then
        ...
        if fam.FrameCount % 20 == 0 then
            GODMODE.sfx:Play(SoundEffect.SOUND_FIRE_BURN)
        end
