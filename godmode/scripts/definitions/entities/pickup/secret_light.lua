local monster = {}
monster.name = "[GODMODE] Secret Light"
monster.type = GODMODE.registry.entities.secret_light.type
monster.variant = GODMODE.registry.entities.secret_light.variant
...
monster.pickup_update = function(self, ent, data, sprite)
    ...
    if sprite:IsFinished("Found") then
        Isaac.Spawn(5,0,0,ent.Position,Vector(0,0),nil)
        ent:Remove()
    end

    if not sprite:IsPlaying("Idle") and not sprite:IsPlaying("Found") or sprite:IsFinished("Appear") then
