local monster = {}
monster.name = "[GODMODE] Fatal Attraction Helper"
monster.type = GODMODE.registry.entities.fatal_attraction_station.type
monster.variant = GODMODE.registry.entities.fatal_attraction_station.variant

local stat_up_mod = 0.125
...
monster.eval_cache = function(self, player,cache,data)
    if render_frame[cache] ~= nil then 
        ...
        local shattered_moonrock = #GODMODE.util.does_player_have(GODMODE.registry.trinkets.shattered_moonrock,true)

        if mod < 0 and shattered_moonrock > 0 then 
            mod = math.min(0,mod+shattered_moonrock*0.025)
        end
