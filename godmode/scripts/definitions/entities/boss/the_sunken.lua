...
monster.name = "[GODMODE] The Sunken"
...
monster.npcUpdate = function(self, ent)
    ...
    if ent:GetSprite():IsEventTriggered("Fire") and ent:GetSprite():IsPlaying("Fire") then--data.time % 30 == 0 or data.time % 30 == 15 then
        ...
        for l=0,num do
            ...
            for i=0,num2 do
                ...
                local t = GODMODE.game:Spawn(EntityType.ENTITY_PROJECTILE,0,ent.Position + ang,ang*spd,ent,0,ent.InitSeed)
            end
        end
    end
    ...
    if ent:GetSprite():IsEventTriggered("Split") and ent:GetSprite():IsPlaying("Split") then
        ...
        if data.Prime == nil then
            local sml = GODMODE.game:Spawn(monster.type,100,ent.Position,Vector(0,0),ent,0,ent.InitSeed)
            getEntData(sml).P3 = true
            sml.HitPoints = ent.MaxHitPoints * 0.3
            local prm = GODMODE.game:Spawn(monster.type,100,ent.Position,Vector(0,0),ent,0,ent.InitSeed)
