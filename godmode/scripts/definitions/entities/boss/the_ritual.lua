...
monster.name = "[GODMODE] The Ritual"
monster.type = GODMODE.registry.entities.the_ritual.type
monster.variant = GODMODE.registry.entities.the_ritual.variant
...
monster.npc_update = function(self, ent, data, sprite)
	...
	if ent.SubType == 1 then
        ...
        if ent.Parent and data.index then
            local master_data = GODMODE.get_ent_data(ent.Parent)
            local off = master_data:get_offset(ent.Parent, ent, data.index)
            local pos = master_data:get_position(ent.Parent, ent, data.index)
            if off and pos then
                ...
                for i=1, #data.streams do
                    ...
                    if str then
                        ...
                        if str.targ_ang ~= nil and str.fire_time > 0 and data.time % str.fire_rate == 0 then
                            ...
                            local fire = Isaac.Spawn(EntityType.ENTITY_PROJECTILE, 2, 0, ent.Position, vec:Rotated(ent:GetDropRNG():RandomFloat() * str.spread - str.spread / 2) * speed, ent):ToProjectile()
                            fire:SetColor(Color(50/255,50/255,255/255,1,100/255,100/255,255/255),999,999,false,false)
                            fire.ProjectileFlags = fire.ProjectileFlags | ProjectileFlags.HIT_ENEMIES
                        end
            ...
            if ent.Parent == nil or ent.Parent:IsDead() or ent.Parent:GetSprite():IsPlaying("Death") then
                sprite:Play("CandleDeath",false)
                data.dead = true
            end
            ...
            master_data:get_anim(ent.Parent, ent, data.index)

            if data.laser then
                if data.laser:IsDead() then 
                    data.laser = nil
                else
                    data.laser.Angle = (ent.Parent.Position - ent.Position):GetAngleDegrees() + 180
                end
            end

            if sprite:IsEventTriggered("Attack") then
                if sprite:IsPlaying("PutOut") then
                    if master_data.cur_attack == 3 then
                        local dir = (ent.Parent.Position - ent.Position):GetAngleDegrees()
                        local ra = math.rad(dir)
                        local vec = Vector(math.cos(ra),math.sin(ra))
                        if data.index <= master_data.phase + 2 then
                            Isaac.Spawn(GODMODE.registry.entities.mum.type, GODMODE.registry.entities.mum.variant, 0, ent.Position + vec * 16, Vector(0,0), ent)
                        end
                    end
                elseif sprite:IsPlaying("CandleDeath") then
                    ent:Kill()
                elseif sprite:IsPlaying("Attack") then
                    ...
                    if master_data.cur_attack == 2 then
                        ...
                        local ra = math.rad(dir)
                        local vec = Vector(math.cos(ra),math.sin(ra))
                        local fire = Isaac.Spawn(EntityType.ENTITY_PROJECTILE,2,0,ent.Position,vec * 10.0,ent):ToProjectile()
                        fire:SetColor(Color(50/255,50/255,255/255,1,100/255,100/255,255/255),999,999,false,false)
                        fire.ProjectileFlags = fire.ProjectileFlags | ProjectileFlags.HIT_ENEMIES
                    end
                elseif sprite:IsPlaying("Overlight") then
                    ...
                    if master_data.cur_attack == 5 then
                        ...
                        str.targ_ang = function()
                            return (ent.Parent.Position - ent.Position):GetAngleDegrees() + 180
                        end
                        str.fire_time = 130
                        str.fire_rate = 2
                        str.spread = 10
                        ...
                        --local l = EntityLaser.ShootAngle(1, ent.Parent.Position, ang, 130, Vector(0,-32), ent.Parent)
                        --data.laser = l
        ...
        if not data.dead then
            if data.lit then
                ...
                elseif data.overlit == true then
                    sprite:Play("Overlight", false)
                end

                if sprite:IsFinished("Overlight") then
                    data.overlit = false
                    sprite:Play("Idle", false)
                end
                ...
                if data.lit_state == 0 then
                    sprite:Play("Light", false)
                end

                if sprite:IsFinished("Light") then
                    data.lit_state = 1
                    sprite:Play("Idle", false)
                end
            else
                if data.lit_state == 1 then
                    sprite:Play("PutOut", false)
                end

                if sprite:IsFinished("PutOut") then
                    data.lit_state = 0
                    sprite:Play("IdleOut")
                end
            end

            if sprite:IsFinished("Appear") and ent.Parent and not ent.Parent:GetSprite():IsFinished("Appear") then
                sprite:Play("Idle", false)
            end
        end
	 -- CANDLE END
	...
	elseif ent.SubType == 0 then
        ...
        if data.candles == nil then
            ...
            if not data.spawned then
                data.spawned = true
                for i=0,4 do
                    local e = Isaac.Spawn(GODMODE.registry.entities.ritual_candle.type, GODMODE.registry.entities.ritual_candle.variant, GODMODE.registry.entities.ritual_candle.subtype, ent.Position, Vector(0,0), ent)
                    e.Parent = ent
        ...
        if perce <= 0.5 and data.phase < 2 and data.attack_time <= 0 then
            data.phase = 2
            data.trans_time = 1
            sprite:Play("Light1", false)
            ent:ToNPC():PlaySound(SoundEffect.SOUND_SATAN_HURT, 1.0, 1, false, 0.9 + ent:GetDropRNG():RandomFloat() * 0.2)
        end
        if perce <= 0.25 and data.phase < 3 and data.attack_time <= 0 then
            data.phase = 3
            data.trans_time = 1
            sprite:Play("Light2", false)
            ent:ToNPC():PlaySound(SoundEffect.SOUND_SATAN_HURT, 1.0, 1, false, 0.9 + ent:GetDropRNG():RandomFloat() * 0.2)
        end

        if data.phase == 1 and sprite:IsFinished("Light0") then
            data.trans_time = 0
        end
        if data.phase == 2 and sprite:IsFinished("Light1") then
            data.trans_time = 0
        end
        if data.phase == 3 and sprite:IsFinished("Light2") then
            data.trans_time = 0
        end
        ...
        if sprite:IsEventTriggered("Attack") then
            ...
            if data.cur_attack == 3 then 
                data.put_out = 120
                if GODMODE.util.count_enemies(nil, GODMODE.registry.entities.silent.type, GODMODE.registry.entities.silent.variant) == 0 then 
                    local silent = Isaac.Spawn(GODMODE.registry.entities.silent.type,GODMODE.registry.entities.silent.variant,0,ent.Position,ent.Velocity * 0.25,ent)
        ...
        if data.attack_time <= 0 and data.trans_time == 0 or sprite:IsFinished(att_s) then
            local idle_s = "Idle"..tostring(data.phase)
            sprite:Play(idle_s, false)
        end
	end
end
...
monster.npc_init = function(self, ent)
    if (GODMODE.room_type or GODMODE.room:GetType()) == RoomType.ROOM_BOSS and GODMODE.level:GetStage() == LevelStage.STAGE5 and GODMODE.level:GetStageType() == StageType.STAGETYPE_ORIGINAL then 
        music_flag = true
    end
end
