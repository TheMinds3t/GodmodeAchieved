local monster = {}
monster.name = "[GODMODE] Brazier (Poky)"
    ...
    if sprite:IsPlaying("EyeFire") then 
        if data.second_sprite then 
            ...
            if sprite:IsEventTriggered("Fire") then
				sfx:Play(SoundEffect.SOUND_GHOST_SHOOT, 1, 0, false, 1)
    ...
    elseif ent.FrameCount > 40 then --idle / spew
        ...
        if in_bounds_flag then 
            if ent.I2 == min_charge then 
                ent.I2 = max_charge
                ent.V1 = move_mask
				sfx:Play(SoundEffect.SOUND_BOSS_LITE_ROAR,1,0,false,1)
        ...
        elseif ent.I2 < max_charge * 0.7 then
			...
			sfx:Play(SoundEffect.SOUND_FORESTBOSS_STOMPS, 1, 0, false, 1)
						--game:MakeShockwave(ent.Position, 0.045, 0.035, 10)
			game:ShakeScreen(15)
