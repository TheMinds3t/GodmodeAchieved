local monster = {}
monster.name = "[GODMODE] Souleater"
...
monster.set_delirium_visuals = function(self,ent)
    for i=0,4 do 
        ent:GetSprite():ReplaceSpritesheet(i,"godmode/gfx/bosses/deliriumforms/souleater2.png")
    end
...
local fire_ring = function(ent, count, off, speed, transform_func, variant, body_off)
    transform_func = transform_func == nil and function(proj, index) end or transform_func
    variant = variant == nil and ProjectileVariant.PROJECTILE_NORMAL or variant
    body_off = body_off == nil and true or body_off

    for i=1, count do 
        local ang = i * (360 / count) + off
        local vel = Vector(1,0):Rotated(ang):Resized(speed)
        local proj = Isaac.Spawn(
            EntityType.ENTITY_PROJECTILE, 
            variant, 
            0, 
            ent.Position+(body_off and fire_circle_off(ent) or Vector.Zero), vel, ent)
        ...
        transform_func(proj, i)
    end
end

local attack_trigger_actions = {
    ["BrimFire"] = {
        fire = function(ent, data, sprite)

        end,
        fire2 = function(ent,data,sprite)
            local dir = ((ent:GetPlayerTarget() or Isaac.GetPlayer()).Position - ent.Position)
            local cnt = 5
            local spread = 360
            local off = ent:GetDropRNG():RandomFloat()*(spread/cnt+1)*2
            local orbit_flag = ProjectileFlags.SINE_VELOCITY
    ...
    ["Charge"] = {
        ...
        fire2 = function(ent,data,sprite) end,
        tell = function(ent,data,sprite) 
            data.tell = ent:GetDropRNG():RandomFloat() * 360 / 5
            data.tell_dir = (ent:GetDropRNG():RandomInt(2) * 2 - 1)
        end
    },
    ["FireLoop"] = {
        fire = function(ent, data, sprite)
            data.tell = data.tell + 16 * (data.tell_dir or 1)
            -- local dist = 16
            -- for i=1,4 do
            --     local ang = data.tell + i * (360 / 4)
            --     local f = math.rad(ang)
            --     local offset = Vector(math.cos(f)*dist,math.sin(f)*dist)
            --     local tell = Isaac.Spawn(GODMODE.registry.entities.unholy_order.type,GODMODE.registry.entities.unholy_order.variant,math.floor(ang),ent.Position+offset+fire_circle_off(ent),Vector.Zero,ent)
            --     local tell_data = GODMODE.get_ent_data(tell)
            --     tell_data.fire_time = 75
            --     tell_data.laser_timeout = 5
            -- end    

            fire_ring(ent, 5, data.tell, 5.25, function(proj, index) 
                proj.Position = proj.Position + proj.Velocity * 85
                proj.Velocity = -proj.Velocity
                -- proj:AddProjectileFlags(ProjectileFlags.MOVE_TO_PARENT)
            end)
        end,
        fire2 = function(ent,data,sprite) 
    ...
    ["FlameAttack"] = {
        fire = function(ent, data, sprite)
            ...
            for l=1,layers do 
                for i=1,cnt do 
                    local speed = min_speed + (max_speed - min_speed)*(l/cnt)
    ...
    ["HeadIdle"] = {
        fire = function(ent,data,sprite) 
            data.tell_idle = (data.tell_idle or 0) + 30
            fire_ring(ent, 4, data.tell_idle % 360, 5.25, function(proj, index) 
                proj.Position = proj.Position + proj.Velocity * 85
                proj.Velocity = -proj.Velocity
                proj:AddProjectileFlags(ProjectileFlags.MOVE_TO_PARENT)
            end, nil, false)
    ...
    ["HeadIdleVulnerable"] = {
        fire = function(ent,data,sprite) 
            data.tell_idle = (data.tell_idle or 0) + 34
            data.idle_alt = not data.idle_alt 

            if data.idle_alt then 
                fire_ring(ent, 4, data.tell_idle % 360, 5.25, function(proj, index) 
                    proj.Position = proj.Position + proj.Velocity * 85
                    proj.Velocity = -proj.Velocity
                    proj:AddProjectileFlags(ProjectileFlags.MOVE_TO_PARENT)
                ...
                fire_ring(ent, 8, data.tell_idle % 360, 4.0, function(proj, index) 
                    proj.Position = proj.Position + proj.Velocity * 120
                    proj.Velocity = -proj.Velocity
                    proj:AddProjectileFlags(ProjectileFlags.MOVE_TO_PARENT)
                end, nil, false)    
            end
    ...
    ["HeadIdleRage"] = {
        fire = function(ent,data,sprite) 
            data.tell_idle = (data.tell_idle or 0) + 50
            data.idle_alt_3 = ((data.idle_alt_3 or 0) + 1) % 3

            if data.idle_alt_3 == 0 then 
                fire_ring(ent, 4, data.tell_idle % 360, 5.25, function(proj, index) 
                    proj.Position = proj.Position + proj.Velocity * 85
                    proj.Velocity = -proj.Velocity
                    proj:AddProjectileFlags(ProjectileFlags.MOVE_TO_PARENT)
                end, nil, false)    
            elseif data.idle_alt_3 == 1 then
                fire_ring(ent, 8, data.tell_idle % 360, 4.0, function(proj, index) 
                    proj.Position = proj.Position + proj.Velocity * 120
                    proj.Velocity = -proj.Velocity
                    proj:AddProjectileFlags(ProjectileFlags.MOVE_TO_PARENT)
                ...
                fire_ring(ent, 6, data.tell_idle % 360, 4.5, function(proj, index) 
                    proj.Position = proj.Position + proj.Velocity * 110
                    proj.Velocity = -proj.Velocity
                    proj:AddProjectileFlags(ProjectileFlags.MOVE_TO_PARENT)
                end, nil, false)    
            end
...
monster.npc_update = function(self, ent, data, sprite)
    ...
    local targ = (ti * 0.5 + (GODMODE.room_center or GODMODE.room:GetCenterPos()) * 2.5) / 3
    ...
    if data.p2 ~= nil and not sprite:IsPlaying("BodyDeath") then
        ...
        if data.guards > 0 and not GODMODE.util.is_delirium() then
            ...
            if ent.HitPoints / ent.MaxHitPoints <= 0.25 then
    ...
    if sprite:IsEventTriggered("Fire") then 
        attack_trigger_actions[sprite:GetAnimation()].fire(ent,data,sprite)
    end

    if sprite:IsEventTriggered("Fire2") then 
        attack_trigger_actions[sprite:GetAnimation()].fire2(ent,data,sprite)
    end

    if sprite:IsEventTriggered("Tell") then 
        attack_trigger_actions[sprite:GetAnimation()].tell(ent,data,sprite)
    end
