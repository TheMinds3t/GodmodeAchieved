local monster = {}
monster.name = "[GODMODE] Bowl Play (Corny)"
monster.type = GODMODE.registry.entities.bowl_play.type
monster.variant = GODMODE.registry.entities.bowl_play.variant
...
monster.npc_update = function(self, ent, data, sprite)
    ...
    if ent.SubType == subs.corny_head or ent.SubType == subs.head then 
        ...
        if not sprite:IsPlaying("Head"..ent.GroupIdx) and not sprite:IsPlaying("Attack"..ent.GroupIdx) and not sprite:IsPlaying("Launch"..ent.GroupIdx) then 
            ...
            if intro_flag and (data.fire_cooldown or 0) == 0 and ent:GetDropRNG():RandomFloat() < 0.5 and #data.body > math.max(1,1+math.floor((ent.HitPoints+ent.MaxHitPoints/2)/ent.MaxHitPoints)-(data.enraged or 0)) then 
                sprite:Play("Launch"..ent.GroupIdx,true)
            else
                ...
                if (data.fire_cooldown or 0) == 1 or intro_flag and (data.spread_mod or 0) == min_spread and ent:GetDropRNG():RandomFloat() < 0.5 then 
                    sprite:Play("Attack"..ent.GroupIdx,true) 
                    data.spread_mod = min_spread
                    data.fire_cooldown = 0
                else
                    sprite:Play("Head"..ent.GroupIdx,true) 
        ...
        if sprite:IsEventTriggered("Attack") then 
            ...
            for i=-count,count do 
                local tear = monster:spawn_flat_tear(ent,
                    ang+i*spread/2.0,
                    2.25+(3-(data.spread_mod or 0))*1.5+(GODMODE.game.Difficulty % 2)*3+(data.enraged or 0),
                    1.125)
            end
        ...
        if sprite:IsEventTriggered("Launch") and #data.body > 1 then 
        ...
        if not sprite:IsPlaying("Body"..(ent.SubType-1)..(ent.Index % 2 + 1)) then 
            sprite:Play("Body"..(ent.SubType-1)..(ent.Index % 2 + 1),true)
        end
    end
end
