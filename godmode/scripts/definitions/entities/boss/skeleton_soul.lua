...
monster.tell = Sprite()
monster.tell:Load("godmode/gfx/ui/chargebar.anm2", true)
...
if not (ent.Type == monster.type and ent.Variant == monster.variant) then return end
	...
	elseif player:ToPlayer() and not sprite:IsPlaying("Death") then
		...
		if count >= timeout and ent:IsFrame(10,0) and not sprite:IsPlaying("Death") then
	...
	if sprite:IsEventTriggered("BloodExplode") then
		ent:BloodExplode()
	end
	...
	if data.finished_intro == true then
		if ent.HitPoints <= 0 then
			...
			sprite:Play("Idle"..tostring(get_phase(perc)), false)
