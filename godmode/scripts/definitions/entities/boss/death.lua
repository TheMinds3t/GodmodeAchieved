...
monster.npc_update = function(self, ent, data, sprite)
    ...
    if sprite:IsEventTriggered("Fire") then 
        ...
        if sprite:IsPlaying("Attack04") then
            GODMODE.sfx:Play(SoundEffect.SOUND_SUMMONSOUND)
        ...
        elseif sprite:IsPlaying("Attack01") then
            --scythes
    ...
    if sprite:IsFinished("Appear") or sprite:IsFinished("Attack01") or sprite:IsFinished("Attack04") then 
    ...
    if ent.HitPoints / ent.MaxHitPoints < 0.5 and not sprite:IsPlaying("DashStart") then 
...
monster.npc_init = function(self,ent)
    if ent.Type == monster.type and ent.Variant == 0 then 
        if StageAPI ~= nil and StageAPI.Loaded and StageAPI.GetCurrentStage() ~= nil and StageAPI.GetCurrentStage().Name == "Colon" or ent:GetDropRNG():RandomFloat() < tonumber(GODMODE.save_manager.get_config("AltHorsemanChance","0.2")) then 
