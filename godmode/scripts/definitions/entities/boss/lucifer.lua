    ...
    if data.time == 1 then
        ...
        if ent.SubType == 8 then
            if ent:GetSprite():IsEventTriggered("Action") then
                GODMODE.game:Spawn(1000, 63, ent.Position, Vector(0,0), player, 0, player.InitSeed)
                GODMODE.game:Spawn(1000, 73, ent.Position, Vector(0,0), player, 0, player.InitSeed)
            end
            ...
            if data.attack_time <= 0 then
                ...
                if data.current_phase == 0 then
                    if data.cur_attack < 0.4 then
                        atc = GODMODE.game:Spawn(800, 141, Vector(-256,-256), Vector(0,0), player, 2, player.InitSeed)
                        data.attack_time = 100 + ent:GetDropRNG():RandomFloat() * 30
                    elseif data.cur_attack < 0.75 then
                        atc = GODMODE.game:Spawn(800, 141, Vector(-256,-256), Vector(0,0), player, 3, player.InitSeed)
                        data.attack_time = 150 + ent:GetDropRNG():RandomFloat() * 30
                    else
                        atc = GODMODE.game:Spawn(800, 141, Vector(-256,-256), Vector(0,0), player, 4, player.InitSeed)
                        data.attack_time = 5000 + ent:GetDropRNG():RandomFloat() * 30
                    end
                elseif data.current_phase == 1 then
                    if data.cur_attack < 0.4 then
                        atc = GODMODE.game:Spawn(800, 141, Vector(-64,-64), Vector(0,0), player, 2, player.InitSeed)
                        data.attack_time = 100 + ent:GetDropRNG():RandomFloat() * 30
                    elseif data.cur_attack < 0.6 then
                        atc = GODMODE.game:Spawn(800, 141, Vector(-256,-256), Vector(0,0), player, 6, player.InitSeed)
                        data.attack_time = 150 + ent:GetDropRNG():RandomFloat() * 30
                    elseif data.cur_attack < 0.8 then
                        atc = GODMODE.game:Spawn(800, 141, Vector(-256,-256), Vector(0,0), player, 4, player.InitSeed)
                        data.attack_time = 5000 + ent:GetDropRNG():RandomFloat() * 30
                    else
                        atc = GODMODE.game:Spawn(800, 141, Vector(-256,-256), Vector(0,0), player, 7, player.InitSeed)
        ...
        if spr:IsPlaying("Eye") then
            ...
            if spr:GetFrame() == 20 and ent.SubType == 2 and getEntData(data.master).current_phase == 1 then
                local e = GODMODE.game:Spawn(800, 141, Vector(-256,-256), Vector(0,0), player, 1, player.InitSeed)
        ...
        if ent.SubType == 3 then
            ...
            elseif data.attack_style < 1.0 then
                if data.time % 5 == 0 then
                    ...
                    local v = Vector(math.cos(math.rad(ang)),math.sin(math.rad(ang)))
                    GODMODE.game:Spawn(800, 200, Isaac.GetRandomPosition() + v * 48, Vector(0,0), player, math.floor(ang), player.InitSeed)
        ...
        if ent.SubType == 6 then
            ...
            if data.time % 5 == 0 then
                ...
                for i=0,2 do
                    ...
                    local v = Vector(math.cos(math.rad(ang)),math.sin(math.rad(ang)))
                    GODMODE.game:Spawn(EntityType.ENTITY_PROJECTILE, 0, Vector(ws[use][1]+ws[use][3],ws[use][2]+ws[use][4]) + v * 16, v * (4 + (i * 1.5)), player, 0, player.InitSeed)
                end
        ...
        if ent.SubType == 7 then
            ...
            if data.time % 2 == 0 and data.attacking then
                ...
                local v = Vector(math.cos(math.rad(ang)),math.sin(math.rad(ang)))
                GODMODE.game:Spawn(EntityType.ENTITY_PROJECTILE, 0, pos - v * 16, v * (4 + ent:GetDropRNG():RandomFloat()), player, 0, player.InitSeed)
            end
